    ...
    <el-container>
      <el-header style="padding: 0px;display:flex;justify-content:space-between;align-items: center">
        <div style="display: inline">
          <el-input
            placeholder="通过新录入人名字搜索员工,记得回车哦..."
            clearable
          ...
          <el-button v-show="false"slot="reference" type="primary" size="mini" style="margin-left: 5px"
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                ...
                <select v-model="select" >
                  <option v-for="(item,index) in status" :key="index" :value="item.stat">{{ item.name }}</option>
                </select>
                <p>更改状态为：{{select}}</p>
                <form method="POST" enctype="multipart/form-data" action="/employee/basic/uploadFile">
                  <p><input type="file" name="file" /></p>
                  <p><input type="submit" value="简历上传" /></p>
                </form>
                <form method="GET" enctype="multipart/form-data" action="/employee/basic/downloadFile">
                  <p><input type="submit" value="简历下载" /></p>
                </form>
                <el-button type="danger" style="padding: 3px 4px 3px 4px;margin: 2px" size="mini"
                           @click="add(scope.row)">加入候选人
                </el-button>
                <el-button type="danger" style="padding: 3px 4px 3px 4px;margin: 2px" size="mini"
